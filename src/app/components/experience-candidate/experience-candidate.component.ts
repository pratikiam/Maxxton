...
@Component({
  ...
  styleUrls: ['./experience-candidate.component.css']
})
export class ExperienceCandidateComponent implements OnInit {

  data:any =[];
  dte:Date = new Date();
  yrs:any = this.dte.getFullYear();
  Doj:any=[]  
  candidate_data = [
  ...
  dataExp(){
    ...
    this.dte.setDate(this.dte.getDate() - 2); 
  
   this.candidate_data.forEach(element => {
     var Doje:any=[]  
    this.Doj = element.joining_date.split('/');  
  
    if(this.yrs - this.Doj[2] > 2){
