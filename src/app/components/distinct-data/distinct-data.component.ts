...
@Component({
  selector: 'app-distinct-data',
  templateUrl: './distinct-data.component.html',
  styleUrls: ['./distinct-data.component.css']
})
export class DistinctDataComponent implements OnInit {
 ...
 data:any= [
  ...
  ngOnInit(): void {
    this.data
   this.distinct(this.data)
  }

  distinct(data){
    console.log(data);
   this.main = this.data.filter(
      (thing, i, arr) => arr.findIndex(t => t.department === thing.department) === i
    );
